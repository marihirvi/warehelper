...
const getAvailability = async (manufacturer) => {
    ...
    if (request.data.response.length > 2) { return request.data } else { return await getAvailability(manufacturer) }
}
...
const getAllAvailabilities = async (manufacturers) => {
    ...
    const promises = await manufacturers.map(async manufacturer => {
        return await getAvailability(manufacturer)
    })
